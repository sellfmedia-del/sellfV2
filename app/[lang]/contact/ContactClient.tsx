...
import { content, expertise, goals, workModes, type Lang, type RouteChoice } from "./content";
  ...
  return <main className={styles.page}>
    <section className={styles.hero} aria-labelledby="contact-title"><div className="sellf-container">
      ...
      <div className={styles.choices}>
        <ChoiceCard type="work" active={choice === "work"} image="/images/contact/work-growth.webp" {...t.workCard} onChoose={choose} />
        <ChoiceCard type="careers" active={choice === "careers"} image="/images/contact/join-team.webp" {...t.careersCard} onChoose={choose} />
      </div>
    </div></section>
...
function ChoiceCard({ type, active, image, label, title, body, cta, onChoose }:{type:RouteChoice;active:boolean;image:string;label:string;title:string;body:string;cta:string;onChoose:(v:RouteChoice)=>void}) {
  return <button type="button" className={`${styles.choiceCard} ${active?styles.choiceActive:""}`} onClick={() => onChoose(type)} aria-pressed={active}>
    <Image src={image} alt="" fill priority sizes="(max-width: 900px) 100vw, 50vw" className={styles.choiceImage}/><span className={styles.choiceOverlay}/><span className={styles.choiceContent}><span className={styles.choiceTop}><span className="sellf-kicker">{label}</span><span className={styles.choiceIndex}>{type==="work"?"01":"02"}</span></span><span><strong>{title}</strong><span className={styles.choiceBody}>{body}</span><span className={styles.choiceCta}>{cta} <Arrow/></span></span></span>
  </button>;
}
...
function CareersRoute({lang,t,status,onSubmit,onSwitch}:{lang:Lang;t:(typeof content)[Lang];status:FormStatus;onSubmit:(e:FormEvent<HTMLFormElement>)=>void;onSwitch:()=>void}) {
  return <><RouteIntro eyebrow={t.careers.eyebrow} title={t.careers.title} body={t.careers.body} switchLabel={t.careers.switch} onSwitch={onSwitch}/><section className={styles.network}><div className="sellf-container"><div className={styles.networkIntro}><p className="sellf-kicker">{t.careers.networkLabel}</p><h2 className="sellf-display">{t.careers.networkTitle}</h2><p>{t.careers.networkBody}</p></div><div className={styles.expertiseGrid}>{expertise[lang].map((item,i)=><div key={item}><span>0{i+1}</span><strong>{item}</strong></div>)}</div><div className={styles.workModes}><div><h3>{t.careers.modesTitle}</h3><p>{t.careers.modesBody}</p></div><div>{workModes.map(mode=><span key={mode}>{mode}</span>)}</div></div></div></section><section className={styles.formSection}><div className="sellf-container"><FormIntro eyebrow={t.careers.formLabel} title={t.careers.formTitle} body={t.careers.formBody}/><form className={styles.form} onSubmit={onSubmit} encType="multipart/form-data"><input type="hidden" name="lang" value={lang}/><Honeypot/><Field label={t.fields.name} name="name" required/><Field label={t.fields.email} name="email" type="email" required/><Field label={t.fields.phone} name="phone" type="tel"/><Field label={t.fields.location} name="location" required/><SelectField label={t.fields.expertise} name="expertise" options={expertise[lang]} required/><Field label={t.fields.portfolio} name="portfolio" type="url" required/><Field label={t.fields.linkedin} name="linkedin" type="url"/><SelectField label={t.fields.model} name="model" options={workModes} required/><Field label={t.fields.availability} name="availability" required/><TextArea label={t.fields.intro} name="intro" required wide/><label className={`${styles.field} ${styles.wide}`}><span>{t.fields.cv}</span><input name="cv" type="file" accept=".pdf,.doc,.docx"/></label><FormFooter status={status} submit={t.careers.submit} success={t.careers.success} sending={t.fields.sending} error={t.fields.error}/></form></div></section></>;
}
...
function Field({label,name,type="text",required=false}:{label:string;name:string;type?:string;required?:boolean}){return <label className={styles.field}><span>{label}{required&&" *"}</span><input name={name} type={type} required={required} maxLength={type==="email"?320:240}/></label>}
