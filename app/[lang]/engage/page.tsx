...
const images = {
  hero: '/images/about-team.webp', watch: 'https://cdn.sellfmedia.workers.dev/statics/video-screnshot.png', attend: '/images/about-team.webp', tools: '/framework/rgi/hero.webp', read: '/framework/rgi/reference-atlas.webp', event: '/framework/rgi/output-exact.svg',
...
export default async function EngagePage({params}: PageProps) {
  ...
  return <div className={styles.page}>
    ...
    <section className={`${styles.darkSection} ${styles.curveDark}`} id="webinars"><div className="sellf-container"><div className={styles.darkHeading}><div><span>{t.webinarEyebrow}</span><h2>{t.webinarTitle}</h2></div><ArrowLink href="#latest" light>{t.allWebinars}</ArrowLink></div><div className={styles.eventGrid}>{webinarItems.map((item, index) => <EventCard key={item.id} item={item} featured={index === 0} register={t.register} />)}</div></div></section>

    <section className={`${styles.whiteSection} ${styles.eventsSection}`} id="events"><div className="sellf-container"><div className={styles.lightHeading}><div><span>{t.eventEyebrow}</span><h2>{t.eventTitle}</h2></div><ArrowLink href="#latest">{t.allEvents}</ArrowLink></div><div className={styles.operatorGrid}>{eventItems.map((item, index) => <OperatorCard key={item.id} item={item} index={index} />)}</div></div></section>

    <section className={`${styles.toolsSection} ${styles.curveBlue}`} id="tools"><div className="sellf-container"><div className={styles.toolsHeading}><div><span>{t.toolsEyebrow}</span><h2>{t.toolsTitle}</h2></div><p>{t.toolsNote}</p><ArrowLink href="#latest" light>{t.allTools}</ArrowLink></div><div className={styles.toolsGrid}>{toolItems.map((item, index) => <Link href={item.href} key={item.id} className={styles.toolCard}><h3>{item.title}<span>→</span></h3><ToolVisual index={index} /><p>{item.summary}</p></Link>)}</div></div></section>

    <section className={`${styles.whiteSection} ${styles.latestSection}`} id="latest"><div className="sellf-container"><div className={styles.latestHeading}><div><span>{t.latestEyebrow}</span><h2>{t.latestTitle}</h2></div><span className={styles.filterPill}>{t.allContent}</span></div><div className={styles.latestGrid}>{contentItems.map((item) => <Link href={item.href} key={item.id} className={styles.latestCard}><div className={styles.latestMedia}><CardImage src={item.image} alt={item.title} sizes="(max-width: 760px) 80vw, 17vw" /><span>{item.label}</span></div><h3>{item.title}</h3><p><span className={styles.avatarDot} />{item.author}</p></Link>)}</div></div></section>

    <section className={`${styles.closing} ${styles.curveClosing}`}><CardImage src={images.hero} alt="" sizes="100vw" /><div className={styles.closingShade} /><div className={`sellf-container ${styles.closingInner}`}><span>{t.closeEyebrow}</span><h2>{t.closeTitle}</h2><p>{t.closeSummary}</p><div><Link href="#webinars" className={styles.primaryButton}>{t.attend}<span>→</span></Link><Link href={`/${lang}/contact`} className={styles.secondaryButton}>{t.work}<span>→</span></Link></div></div></section>
  </div>
}
